...
def evaluate(hps, argx, ctx):
  """Eval loop. (Don't care about distributing this for now)"""
  images, labels = cifar_input.build_input(
      args.dataset, args.eval_data_path, hps.batch_size, args.mode, 0, 1)
  model = resnet_model.ResNet(hps, images, labels, args.mode)
  model.build_graph()
  ...
  while True:
    ...
    if not (ckpt_state and ckpt_state.model_checkpoint_path):
      tf.logging.info('No model to eval yet at %s', args.log_root)
      continue
    tf.logging.info('Loading checkpoint %s', ckpt_state.model_checkpoint_path)
    saver.restore(sess, ckpt_state.model_checkpoint_path)

    total_prediction, correct_prediction = 0, 0
    for _ in six.moves.range(args.eval_batch_count):
